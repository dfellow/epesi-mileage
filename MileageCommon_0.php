...
class Custom_MileageCommon extends ModuleCommon {
	public static function applet_caption() {
		if (Utils_RecordBrowserCommon::get_access('mileage','browse'))
			return __('Mileage');
	}

	public static function applet_info() {
		return __('Mileage tracking');
	}
	...
	public static function applet_info_format($r){
		...
		$args=array(
					__('Mileage')=>'<b>'.$r['miles'].'</b>',
					__('Vehicle')=>$vehicle[$r['vehicle']],
					...
					__('Fuel')=>$r['fuel']!=0?__('Yes'):__('No'),
					__('Receipt')=>$r['receipt']!=0?__('Yes'):__('No')
					);
	...
	public static function menu() {
		if (Utils_RecordBrowserCommon::get_access('mileage','browse'))
			return array(_M('Custom')=>array('__submenu__'=>1,_M('Mileage')=>array()));
		else
			return array();
	}
	...
	public static function task_bbcode($text, $param, $opt) {
		return Utils_RecordBrowserCommon::record_bbcode('mileage', array('title'), $text, $param, $opt);
	}
	
	*/
	public static function get_mileages($crits = array(), $cols = array(), $order = array()) {
		return Utils_RecordBrowserCommon::get_records('mileage', $crits, $cols, $order);
	}

	public static function get_mileage($id) {
		return Utils_RecordBrowserCommon::get_record('mileage', $id);
	}
	...
	*/
    ...
    public static function display_miles($record, $nolink) {
		$ret = Utils_RecordBrowserCommon::create_linked_label_r('mileage', 'Miles', $record, $nolink);
		if (isset($record['Miles']) && $record['Miles']!='' && !MOBILE_DEVICE) $ret = '<span '.Utils_TooltipCommon::open_tag_attrs(Utils_RecordBrowserCommon::format_long_text($record['Miles']), false).'>'.$ret.'</span>';
		return $ret;
	}
    public static function display_miles_with_mark($record) {
		return self::display_miles($record, false);
	}
	/*
	public static function display_status($record, $nolink, $desc) {
		$prefix = 'crm_mileage_leightbox';
		$v = $record[$desc['id']];
		if (!$v) $v = 0;
		...
		if (!Utils_RecordBrowserCommon::get_access('mileage', 'edit', $record) && !Base_AclCommon::i_am_admin()) return $status[$v];
		if (isset($_REQUEST['form_name']) && $_REQUEST['form_name']==$prefix.'_follow_up_form' && $_REQUEST['id']==$record['id']) {
			...
			if ($note) {
				...
				$note = str_replace("\n",'<br />',$note);
				Utils_AttachmentCommon::add('mileage/'.$record['id'],0,Acl::get_user(),$note);
			}

			if ($action == 'set_in_progress') $v = 1;
			Utils_RecordBrowserCommon::update_record('mileage', $record['id'], array('status'=>$v));
			...
			if ($action != 'none') {		
				...
				//$values['follow_up'] = array('mileage',$record['id'],$record['title']);
				if ($action == 'new_mileage') $x->push_main('Utils/RecordBrowser','view_entry',array('add', null, $values), array('mileage'));
	...
	*/
	...
	public static function subscribed_employees($v) {
		...
		foreach ($v['employees'] as $k) {
			...
			if ($user!==false && $user!==null) Utils_WatchdogCommon::user_subscribe($user, 'mileage', $v['id']);
		}
	}
	*/
	...
	public static function submit_task($values, $mode) {
		...
		switch ($mode) {
		case 'display':
			//$values['miles'] = __('Follow-up').': '.$values['miles'];
			//$values['status'] = 0;
			...
			$ret['new']['mileage'] = '<a '.Utils_TooltipCommon::open_tag_attrs(__('New Mileage')).' '.Utils_RecordBrowserCommon::create_new_record_href('mileage', $values).'><img border="0" src="'.Base_ThemeCommon::get_template_file('Custom_Mileage','icon-small.png').'" /></a>';
			...
			$ret['new']['note'] = Utils_RecordBrowser::$rb_obj->add_note_button('mileage/'.$values['id']);
			return $ret;
		case 'adding':
		...
		case 'add':
			break;
		case 'edit':
			$old_values = Utils_RecordBrowserCommon::get_record('mileage',$values['id']);
		...
		case 'added':
			...
			foreach ($related as $v) {
				...
				foreach($subs as $s)
					Utils_WatchdogCommon::user_subscribe($s, 'mileage',$values['id']);
			}
			*/
			break;
		}
	...
	public static function search_format($id) {
		if(!Utils_RecordBrowserCommon::get_access('mileage','browse')) return false;
		$row = self::get_mileages(array('id'=>$id));
		if(!$row) return false;
		$row = array_pop($row);
		return Utils_RecordBrowserCommon::record_link_open_tag('mileage', $row['id']).__( 'Mileage (attachment) #%d, %s', array($row['id'], $row['title'])).Utils_RecordBrowserCommon::record_link_close_tag();
	}
	...
	*/
	...
	public static function mobile_menu() {
		if(!Utils_RecordBrowserCommon::get_access('mileage','browse'))
			return array();
		return array(__('Mileage')=>array('func'=>'mobile_mileage','color'=>'blue'));
	}
	...
	*/
	...
	public static function crm_view_event($id, $cal_obj) {
		$rb = $cal_obj->init_module('Utils_RecordBrowser', 'mileage');
